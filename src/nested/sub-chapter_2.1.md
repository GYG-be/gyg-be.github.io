...
Opportunities: If so inclined to explore RustLang [or similar low-level languages] to build components that enable the extension to hybrid setups with Raspberry Pi clusters [or similar hdw] for redundancy or edge computing, or integrate with super-affordable ARM-based boards that might be even better than NVIDIA Jetson for GPU-accelerated local inference without breaking the bank.
